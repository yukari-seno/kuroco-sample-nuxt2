...
$(function(){
    ...
    $('.select-choice').on('click', function() {
        var SelChoice = $(this).next(".select-choice-wrap");
        $(SelChoice).slideToggle();
...
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
    ...
    let animeOption;
    const accordionToggleOnClick = e => {
        e.preventDefault();
        const tg = e.currentTarget;
        const rel = tg.dataset.accordionToggle;
        const isActive = tg.classList.contains(classActive);
        const cts = rel === '' ? tg.parentNode.nextElementSibling : document.querySelector(`[data-accordion-cts="${rel}"]`);
        if (!cts) return;
        if (isActive) {
            tg.classList.remove(classActive);
            (0, _slideToggle_js__WEBPACK_IMPORTED_MODULE_2__.slideUp)(cts, animeOption);
            const event = new CustomEvent('accordion.close.complete');
            ...
            const event = new CustomEvent('accordion.open.complete');
            tg.dispatchEvent(event);
        }
    };
    const accordionToggleInit = el => {
    ...
    const accordion = (selector = '[data-accordion-toggle]', args = {}) => {
    ...
    const getElementHeight = el => {
        if (!el) return;
        const leCopy = el.cloneNode(true);
    
        // Radioボタンの場合値がクリアされるため、グルーピングを変える
        Array.from(leCopy.querySelectorAll('input'), i => {
        ...
        const copyH = leCopy.offsetHeight;
    ...
    const slideUp = (el, args = {}) => {
        const _opt = Object.assign({}, defaultOptions, args);
        _opt.height = `0px`;
        _opt.targets = el;
        _opt.begin = function () {
        ...
        _opt.complete = function () {
            ...
            const event = new CustomEvent('slideup.complete');
            el.dispatchEvent(event);
        };
        (0, animejs__WEBPACK_IMPORTED_MODULE_1__["default"])(_opt);
    };
    const slideDown = (el, args = {}) => {
        const _h = (0, _getElementHeight_js__WEBPACK_IMPORTED_MODULE_2__.getElementHeight)(el);
        const _opt = Object.assign({}, defaultOptions, args);
        _opt.height = `${_h}px`;
        _opt.targets = el;
        _opt.begin = function () {
        ...
        _opt.complete = function () {
            ...
            const event = new CustomEvent('slidedown.complete');
            el.dispatchEvent(event);
        };
        (0, animejs__WEBPACK_IMPORTED_MODULE_1__["default"])(_opt);
    };
    ...
    const tabChange = e => {
        ...
        const group = el.dataset.tabBtn;
        const tabs = document.querySelectorAll(`[data-tab-btn="${group}"]`);
        const rel = el.getAttribute('href').replace(/.*#/, '');
        const cts = document.querySelectorAll(`[data-tab-cts="${group}"]`);
        const tg = document.querySelector(`#${rel}`);
        const root = document.querySelector(`[data-tab-root="${group}"]`);
    ...
    const tabNav = (selector = '[data-tab-btn]') => {
    ...
    const evClick = e => {
        e.preventDefault();
        const el = e.currentTarget;
        const rel = el.dataset.readmoreToggle;
        const tg = document.querySelector(`[data-readmore-cts="${rel}"]`);
        el.classList.add(classActive);
        tg.classList.add(classActive);
    };
    const readmoreInit = el => {
        const rel = el.dataset.readmoreToggle;
        el.addEventListener('click', evClick, false);
    };
    const readmore = (selector = '[data-readmore-toggle]') => {
        const els = Array.prototype.slice.call(document.querySelectorAll(selector));
        for (let i = 0, l = els.length; i < l; i++) {
            const el = els[i];
            readmoreInit(el);
